            ...
            if (result == 0x01)
            {
                ...
                else if (type == 0x02)
                {
                    if (packet.ReadUInt8() == 0x30)
                    {
                        ...
                        if (attacker_id == Caracter.UniqueID)
                        {
                            //Skill casted !
                            packet.ReadUInt32();
                            packet.ReadUInt32();
                        ...
                        else
                        {
                            ...
                            if ((obj_id == Caracter.UniqueID) || (obj_id == Data.char_attackpetid && Globals.MainWindow.Checked(Globals.MainWindow.protectpet) == true))
                            {
     ...
     /*   static void Skills_Elapsed(object sender, ElapsedEventArgs e, Skill skill)
       ...
       public static void CheckSkills()
        ...
        public static void CastSkill(Skill skill, uint mob_id)
        {
            if (skill.SType == 1)
            {
                Packet NewPacket = new Packet(Opcode.CLIENT_OBJECTACTION);
                NewPacket.WriteUInt8(1);
                NewPacket.WriteUInt8(4);
