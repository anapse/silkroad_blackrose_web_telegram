...
namespace bot.Clases.infoclass
{
    class Spawn
    {
        ...
        public static void SingleDeSpawn(Packet packet)
        {
            ...
            foreach (Item item in Item.SpawnItem)
            {
                Console.WriteLine("Item.SpawnItem.Count: " + Item.SpawnItem.Count);
                if (id == item.UniqueID)
                {
                    Item.SpawnItem.Remove(item);
                    break;
            ...
            foreach (Item item in Item.PickableItem)
            {
                Console.WriteLine("Item.PickableItem.Count: " + Item.PickableItem.Count);
                Console.WriteLine("despaun item");
                if (id == item.UniqueID)
                {
                    Item.PickableItem.Remove(item);
                    break;
