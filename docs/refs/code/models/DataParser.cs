    ...
    class DataParser
    {
        ...
        public static void ParseMob(Packet packet, int index)
        {
            try
            {
                ...
                int Distance = (int)(Math.Abs((X - Caracter.X)) + Math.Abs((Y - Caracter.Y)));
           ...
           // Console.WriteLine("parseitem");
            try
            {
                ...
                if (!type.StartsWith("ITEM_QSP") || !type.StartsWith("ITEM_QNO"))
                {
                    ...
                    {
                        if (packet.ReadUInt32() == Caracter.AccountID) // Owner ID
                        {
                ...
                if (packet.Opcode == 0x3015)
                {
                    ...
                    switch (Source)
                    {
                        case 5:
                            packet.ReadUInt32();
                            Console.WriteLine("  //Dropper ID - Monster");
                            break;
                        case 6:
                            packet.ReadUInt32();
                            Console.WriteLine(" //Dropper ID - Player");
                            break;
                    }
                }
            }
        ...
        public static void ParseChar(Packet packet, int index)
        {
            try
            {
                ...
                //Console.WriteLine(UniqueID + " uniq id desde parsechar");
              ...
              //  Console.WriteLine(name + "   dfdsfmsdñfmsdf");
        ...
        {
            Console.WriteLine("este es el inde" + index);
            try
            {
                int s_index = 0;
