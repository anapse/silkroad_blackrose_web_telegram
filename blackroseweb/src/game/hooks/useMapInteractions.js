  ...
  const handleMapClick = (e, zoomLevel) => {
    ...
    setPlayers(prev => {
      ...
      if (!isCity) {
        ...
        const rX = Math.floor(clickWX / R);
        const rZ = Math.floor(clickWZ / R);
      ...
      if (wsSend && !isCity) {
        const targetRX = Math.floor(tWX / R);
        const targetRZ = Math.floor(tWZ / R);
        const targetRegion = targetRX | (targetRZ << 8);
        const targetLocalX = Math.round(tWX - targetRX * R);
        const targetLocalZ = Math.round(tWZ - targetRZ * R);
        wsSend({
          type: 'MOVE',
          region: targetRegion,
          posX: targetLocalX,
