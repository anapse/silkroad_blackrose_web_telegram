...
namespace bot.Clases.infoclass
{
    class Caracter
    {
        public static BotConfigViewModel BotConfigViewModel => App.BotConfigViewModel ?? new BotConfigViewModel();
        public static PlayerViewModel PlayerViewModelInstance => App.GlobalPlayerViewModel ?? new PlayerViewModel();
        ...
        public static void CharacterList(Packet packet)
        {
            ...
            else if (type == 2)
            {
                if (packet.ReadUInt8() == 1)
                {
                    ...
                    for (int i = 0; i < charcount; i++)
                    {
                        ...
                        string caracters = packet.ReadAscii(); // Name
                        newCharacters.Add(caracters);
                        ...
                        Console.WriteLine("cantidad de item en charlist = " + itemscount);
            ...
            Application.Current?.Dispatcher.Dispatch(() =>
            {
                ...
                {
                    App.MainViewModel?.Characters.Add(character);
                }
            });
        }
        public struct CAVE_
        ...
        {
            Caracter.UniqueID = packet.ReadUInt32();
        }
        public static void CharData(Packet packet)
        {
            try
            {
                Data.inventoryicon.Clear();
                ...
                packet.ReadUInt32();//1
                PlayerViewModelInstance.RefObjID = (int)(Caracter.RefObjID = packet.ReadUInt32());//2
                Caracter.Scale = packet.ReadUInt8();//3
                PlayerViewModelInstance.Level = (Caracter.Level = packet.ReadUInt8()).ToString();//4
                Caracter.MaxLevel = packet.ReadUInt8();//5
                int maxlvl = (int)Caracter.Level - 1;
                Caracter.ExpMax = Convert.ToUInt64(Caracter.explist[maxlvl]);
                PlayerViewModelInstance.ExpOffset = Caracter.ExpOffset = packet.ReadUInt64();//6
                Caracter.SExpOffset = packet.ReadUInt32();//7
                PlayerViewModelInstance.Gold = (Caracter.Gold = packet.ReadUInt64()).ToString();//8
                PlayerViewModelInstance.SkillPoints = (Caracter.SP = packet.ReadUInt32()).ToString();//9
                Caracter.RemainStatPoint = packet.ReadUInt16();//10
                Caracter.BerserkLevel = packet.ReadUInt8();//11
                Caracter.GatheredExpPoint = packet.ReadUInt32();//12
                PlayerViewModelInstance.HP = (Caracter.CurrentHP = packet.ReadUInt32()).ToString();//13
                PlayerViewModelInstance.MP = (Caracter.CurrentMP = packet.ReadUInt32()).ToString(); ;//14
                Caracter.AutoInvestExp = packet.ReadUInt8();//15
                Caracter.DailyPK = packet.ReadUInt8();//16
                Caracter.TotalPK = packet.ReadUInt16();//17
                Caracter.PKPenaltyPoint = packet.ReadUInt32();//18
                packet.ReadUInt8();//19
                packet.ReadUInt8();//20
                PlayerViewModelInstance.Inventorysize = Caracter.InventorySize = packet.ReadUInt8(); //21
                Caracter.InventoryItemsCount = packet.ReadUInt8();//22
                for (int i = 0; i < Caracter.InventoryItemsCount; i++)
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        Data.inventoryicon.Add(Items_Info.items_iconlist[index]);
                        ...
                        DataItemInventory itemData = new DataItemInventory
                        {  
                            ItemRefItemID = ItemRefItemID,
                            Name = Items_Info.itemsnamelist[index],
                            CodeName = Items_Info.itemstypelist[index],
                            Icon = Items_Info.items_iconlist[index],
                            Level = Items_Info.itemslevellist[index],
                            MaxDurability = Items_Info.itemsdurabilitylist[index],
                            Durability = 0, 
                            OptLevel = 0,     
                            Variance = 0,
                            MagParamNum = 0
                        };
                        if (type.StartsWith("ITEM_CH") || type.StartsWith("ITEM_EU") || type.StartsWith("ITEM_MALL_AVATAR") || type.StartsWith("ITEM_ETC_E060529_GOLDDRAGONFLAG") || type.StartsWith("ITEM_EVENT_CH") || type.StartsWith("ITEM_EVENT_EU") || type.StartsWith("ITEM_EVENT_AVATAR_W_NASRUN") || type.StartsWith("ITEM_EVENT_AVATAR_M_NASRUN") || type.StartsWith("ITEM_BR_MALL"))
                        {
                          
                            itemData.OptLevel = packet.ReadUInt8();
                            itemData.Variance = packet.ReadUInt64();
                            itemData.Durability = packet.ReadUInt32();
                            Data.inventorydurability.Add(itemData.Durability);
                            byte MagParamNum = packet.ReadUInt8();
                            itemData.MagParamNum = MagParamNum;
                        ...
                        {
                            // Console.WriteLine("aki esta el pet type " + type);
                            byte Status = packet.ReadUInt8(); //1 =>  Unsumonned, 2 => Summonned, 3 => Alive, 4 => Dead
                            if (Status == 2 || Status == 3 || Status == 4)
                            {
                                uint PetID = packet.ReadUInt32();
                            ...
                            Data.inventorycount.Add(1);
                            Data.inventorydurability.Add(0);
                            // Console.WriteLine("entro en items  3 grap pet ");
                        }
                        else if (type == "ITEM_ETC_TRANS_MONSTER")
                        {
                            ...
                            itemData.Count = 1;
                            Data.inventorydurability.Add(0);
                            // Console.WriteLine("entro en items  4 mionster");
                        }
                        else if (type.Contains("ITEM_ETC_ARCHEMY_ATTRSTONE") || type.Contains("ITEM_ETC_ARCHEMY_MAGICSTONE"))
                        {
                            ...
                            Data.inventorydurability.Add(0);
                            itemData.Count = StoneStackCount;
                            //  Console.WriteLine("entro en items   5 stones");
                        }
                        ...
                        else if (type.Contains("ITEM_MALL_GACHA_CARD_WIN") || type.Contains("ITEM_MALL_GACHA_CARD_LOSE"))
                        {
                            ...
                            Data.inventorydurability.Add(0);
                            itemData.Count = CouponStackCount;
                            // Console.WriteLine("entro en items  7  magic card  ");
                        }
                        else
                        {
                            // Console.WriteLine("entro en items  8 la ultima");
                            ushort ItemStackCount = packet.ReadUInt16();
                            Data.inventorycount.Add(ItemStackCount);
                            itemData.Count = ItemStackCount;
                            Data.inventorydurability.Add(0);
                        }
                      Data.inventory.Add(ItemSlot, itemData);
                    }
                    
                }
                  ItemsCount.CountManager();
                Caracter.AvatarInventorySize = packet.ReadUInt8();
                Caracter.AvatarInventoryItemCount = packet.ReadUInt8();
                for (int i = 0; i < Caracter.AvatarInventoryItemCount; i++)
                {
                ...
                packet.ReadUInt8();
                Caracter.MasteryFlag = packet.ReadUInt8(); //0 = done, 1 = mastery
                while (MasteryFlag == 1)
                {
                ...
                packet.ReadUInt8();
                Caracter.SkillFlag = packet.ReadUInt8(); //0 = done, 1 = mastery
                while (SkillFlag == 1)
                {
                ...
                charid.WriteUInt32(Caracter.UniqueID);
                charid.Lock();
                ...
                int indexx = Caracter.locationsector.IndexOf(sector);
                if (indexx != -1)
                {
                    string location = Caracter.location[indexx];
                    //  PortConfigs.TrainWindow.Label(PortConfigs.TrainWindow.location, location);
                }
                BotConfigViewModel.X_cord = Caracter.X.ToString();
                BotConfigViewModel.Y_cord = Caracter.Y.ToString();
                packet.ReadUInt8();
                packet.ReadUInt8();
                ...
                packet.ReadUInt8();
                Caracter.WalkSpeed = packet.ReadSingle();
                Caracter.RunSpeed = packet.ReadSingle();
                Caracter.BerserkSpeed = packet.ReadSingle();
                ...
                PlayerViewModelInstance.PlayerName = Caracter.PlayerName = packet.ReadAscii();
                ...
                packet.ReadUInt8();
                packet.ReadUInt8();
                Caracter.AccountID = packet.ReadUInt32();
                PlayerViewModelInstance.UpdateInventory();

                if (data_loaded == 0)
                {
                    data_loaded = 1;
                    Data.Statistic.sp_begin = (int)Caracter.SP;
                    Data.Statistic.gold_begin = (long)Caracter.Gold;
                    //  Configs.ReadConfigs();
                ...
                {
                    StartLooping.Start();
                    Console.WriteLine("avisar si el bot se prende  ");
                }
            }
            catch (IndexOutOfRangeException ex)
            {
                Console.WriteLine($"Error en: {ex.StackTrace}");
                Console.WriteLine($"Mensaje: {ex.Message}");
                Console.WriteLine("ubo un error ");
            }
            catch (EndOfStreamException ex)
            {
                Console.WriteLine($"Error en: {ex.StackTrace}");
                Console.WriteLine($"Mensaje: {ex.Message}");
                Console.WriteLine("Se alcanzó el final del flujo de datos");
            }

            
            Packet packet5 = new Packet(0x3012);
            Agent.Send(packet5);
        }
        ...
        static void time_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            System.Threading.Thread.Sleep(10);
            int total_sp = (int)Caracter.SP - Data.Statistic.sp_begin;
            System.Threading.Thread.Sleep(10);
        ...
        public static void UpdateInfo(Packet packet)
        {
            ...
            switch (code)
            {
                case 1:
                    ...
                    Caracter.Gold = packet.ReadUInt64();
                    PlayerViewModelInstance.Gold = Caracter.Gold.ToString();
                ...
                case 4:
                    ...
                    Caracter.BerserkLevel = packet.ReadUInt8();
        ...
        public static void CaracterInfo(Packet packet)
        {
            {
                packet.ReadUInt64();
                packet.ReadUInt64();
                ...
                packet.ReadUInt16();
                packet.ReadUInt16();
                Caracter.MaxHP = packet.ReadUInt32();
                Caracter.MaxMP = packet.ReadUInt32();
                Caracter.STR = packet.ReadUInt16();
                Caracter.INT = packet.ReadUInt16();
        ...
        public static void ExpSpUpdate(Packet packet)
        {
            ...
            if (Caracter.ExpOffset + exp >= Caracter.ExpMax)
            {
                int maxlvl = (int)Caracter.Level - 2;
                ulong exp_max = Convert.ToUInt64(Caracter.explist[maxlvl]);
                ulong new_exp = (Caracter.ExpOffset + exp) - exp_max;
                Caracter.ExpOffset = new_exp;
            ...
            {
                Caracter.ExpOffset = Caracter.ExpOffset + exp;
                // Globals.MainWindow.SetText(Globals.MainWindow.currentexp, Convert.ToString(Character.ExpOffset));
                // Globals.MainWindow.SetText(Globals.MainWindow.exppercent, String.Format("{0}%", Character.ExpOffset * 100 / ExpMax));
            }
