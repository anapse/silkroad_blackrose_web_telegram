...
namespace bot.Clases.Logica.Training
{
    class StorageControl
    {
        ...
        {
            Packet NewPacket = new Packet(Opcode.CLIENT_STORAGE_DATA_REQUEST);
            NewPacket.WriteUInt32(id);
        ...
        public static void OpenStorage1()
        {
            ...
            {
                Packet NewPacket = new Packet(Opcode.CLIENT_ENTITY_TALK_REQUEST);
                if (Data.selectednpctype != null)
                {
        ...
        {/*
            if (Data.bot && Data.loop)
            {
                if (true)
                {
                    Packet NewPacket = new Packet(Opcode.CLIENT_NPC_CLOSE_REQUEST);
                    NewPacket.WriteUInt32(Spawns.NPCID[Spawns.NPCType.IndexOf(Data.selectednpctype)]);
                    Agent.Send(NewPacket);
                }
                else if (true)
                {
                    for (byte i = 13; i < Caracter.InventorySize; i++)
                    {
