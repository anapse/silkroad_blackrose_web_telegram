                ...
                if (Data.bot)
                {
                    if (i < count)
                    {
                        ...
                        try
                        {
                            ...
                            if (action != null && action.StartsWith("go"))
                            {
                                Data.loopaction = "go";
                                Coordinates.x = Convert.ToInt32(action.Split('(')[1].Split(')')[0].Split(',')[0]);
                                Coordinates.y = Convert.ToInt32(action.Split('(')[1].Split(')')[0].Split(',')[1]);
                                int dist = (int)(Math.Abs(Convert.ToInt32(action.Split('(')[1].Split(')')[0].Split(',')[0]) - Caracter.X) + Math.Abs(Convert.ToInt32(action.Split('(')[1].Split(')')[0].Split(',')[1]) - Caracter.Y));
                                ...
                                {
                                    time = (dist / Convert.ToInt64(Caracter.RunSpeed * 0.08));
                                }
                                else
                                {
                                    time = (dist / Convert.ToInt64(Data.char_horsespeed * 0.08));
                                }
                                System.Timers.Timer repeat = new System.Timers.Timer();
                            ...
                            if (action != null && action.StartsWith("talk"))
                            {
                            ...
                            if (action != null && action.StartsWith("delay"))
                            {
                            ...
                            if (action != null && action.StartsWith("teleport"))
                            {
                            ...
                            if (action != null && action.StartsWith("set"))
                            {
                        ...
                        if (i == count)
                        {
                            ...
                            if (Data.loopend == 0)
                            {
                                Controles.InventoryControl.MergeItems();
